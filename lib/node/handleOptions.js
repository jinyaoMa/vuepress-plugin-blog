...
function handleOptions(options, ctx) {
  ...
  directories = directories.filter(directory => {
    const targetDir = shared_utils_1.path.join(
      ctx.sourceDir,
      directory.dirname
    );
    ...
    shared_utils_1.logger.warn(
      `[@vuepress/plugin-blog] Invalid directory classifier: ${shared_utils_1.chalk.cyan(
        directory.id
  ...
  for (const directory of directories) {
    ...
    paginations.push({
      ...
      ...util_1.resolvePaginationConfig(
        Classifier_1.ClassifierTypeEnum.Directory,
        globalPagination,
        pagination,
        indexPath,
        ctx
  ...
  if (options.comment) {
    ...
    switch (commentService) {
      ...
      default:
        shared_utils_1.logger.warn(
          `[@vuepress/plugin-blog] Invalid comment service: ${shared_utils_1.chalk.cyan(
            commentService
          )}`
        );
        break;
